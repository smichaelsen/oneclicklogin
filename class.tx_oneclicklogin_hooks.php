...
class tx_oneclicklogin_hooks {
	...
	protected function generateRecord() {
		...
		while ($beUser = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			...
			$tx_oneclicklogin_users[] = '<a href="#" onclick="TYPO3BackendLogin.switchToOpenId();$(\'t3-username\').value=\'' . htmlspecialchars($beUser['tx_openid_openid']) . '\';$(\'t3-login-form-outer\').parentNode.submit();TYPO3BackendLogin.showLoginProcess();return false;">' . $name . '</a>';
		}
